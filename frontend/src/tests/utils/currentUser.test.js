...
describe("utils/currentUser tests", () => {
    ...
    describe("hasRole tests", () => {
        ...
        test('hasRole(x,"ROLE_USER") returns correct values when currentUser properly defined', async () => {
            ...
            expect(hasRole({ loggedIn: true, root: { rolesList: ["ROLE_USER"] } }, "ROLE_USER")).toBeTruthy();
            expect(hasRole({ loggedIn: true, root: { rolesList: ["ROLE_USER", "ROLE_ADMIN"] } }, "ROLE_USER")).toBeTruthy();
        });
